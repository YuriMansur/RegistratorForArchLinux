...
class TrendsWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        # Пресеты (только для ручного диапазона)
        self._preset_buttons: list[QPushButton] = []
        for label, secs in [("1ч", 3600), ("6ч", 21600), ("24ч", 86400), ("7д", 604800)]:
            btn = QPushButton(label)
            btn.setFixedWidth(40)
            btn.clicked.connect(lambda _, s=secs, b=btn: self._select_preset(s, b))
            arch_layout.addWidget(btn)
    ...
    # Панель каналов
        ...
        # Установка максимальной ширины и расширение по вертикали для адаптивного интерфейса
        ch_frame.setSizePolicy(QSizePolicy.Policy.Maximum, QSizePolicy.Policy.Expanding)
    ...
    # Строка управления над списком
        ...
        btn_points  = QPushButton("•")
        # Установка размеров кнопок
        btn_all.setFixedSize(24, 22)
        btn_none.setFixedSize(24, 22)
        btn_points.setFixedSize(24, 22)
        # Установка подсказок для кнопок строки управления
        btn_points.setToolTip("Точки для всех")
        btn_all.setToolTip("Выбрать все")
        btn_none.setToolTip("Сбросить все")
        # Подключение сигналов кнопок к соответствующим слотам
        btn_all.clicked.connect(self._select_all)
        btn_none.clicked.connect(self._select_none)
        btn_points.toggled.connect(self._set_all_points)
        btn_points.setCheckable(True)
        btn_all.setStyleSheet(_CTRL_BTN_STYLE)
        btn_none.setStyleSheet(_CTRL_BTN_STYLE)
        btn_points.setStyleSheet(_CTRL_BTN_STYLE)
        # Добавляет кнопки в строку управления
    ...
    # Заголовок панели каналов
        ...
        # Вертикальная прокрутка появляется при необходимости, 
        self._ch_scroll.setWidgetResizable(True)
        # Отключение горизонтальной прокрутки, так как она не нужна для списка каналов
        self._ch_scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
        # Скрытие рамки вокруг области прокрутки для более чистого внешнего вида
    ...
    def _load_archive(self):
        ...
        try:
            rows = api_client.get_history_range(from_dt, to_dt, visible_tags)
        except Exception as e:
            print(f"[trends] ошибка загрузки: {e}")
            return
